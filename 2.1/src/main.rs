...
fn read_input(path: &str) -> Result<Vec<PasswordCheck>> {
    ...
    for (num, line) in reader.lines().enumerate() {
        ...
        output.push(PasswordCheck {
            min: caps.get(1).ok_or(anyhow!("min not found on {}", num + 1))?.as_str().parse()?,
            max: caps.get(2).ok_or(anyhow!("max not found on {}", num + 1))?.as_str().parse()?,
